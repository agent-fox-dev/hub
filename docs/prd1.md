...
`POST /api/v1/workspaces` accepts two additional optional fields in the request body: `display_name` and `description`. Both default to `null` if omitted.
...
- **Schema migration:** Pre-production; the new columns are added via `ALTER TABLE` or recreate in the `initSchema` DDL. No migration framework.

## Dependencies

| Package | Purpose |
|---------|---------|
| `github.com/txsvc/apikit` | Permission registry, auth middleware, server framework |
